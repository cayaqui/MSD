...
public class Variation : BaseEntity, IAuditable, ISoftDelete, ICodeEntity
{
    ...
    public ICollection<VariationAttachment> Attachments { get; private set; } =
        new List<VariationAttachment>();
    ...
    )
    {
        ...
        ProjectId = projectId;
        ContractReference =
            contractReference ?? throw new ArgumentNullException(nameof(contractReference));
        Type = type;
        Category = category;
    ...
    public void SetScheduleImpact(
        int? extensionDays,
        DateTime? revisedCompletion,
        bool criticalPathImpacted
    )
    {
    ...
    public void Reject(Guid rejectedBy, string reason)
    {
        if (Status == VariationStatus.Approved || Status == VariationStatus.Implemented)
            throw new InvalidOperationException(
                "Cannot reject an approved or implemented variation"
            );
    ...
    public void RecordCertification(decimal certifiedAmount)
    {
        if (!IsImplemented)
            throw new InvalidOperationException(
                "Variation must be implemented before certification"
            );
