...
public class Trend : BaseEntity, IAuditable, ISoftDelete, ICodeEntity
{
    ...
    public decimal? ExpectedValue =>
        MostLikelyCostImpact.HasValue ? MostLikelyCostImpact.Value * (Probability / 100) : null;
    ...
    public ICollection<TrendAttachment> Attachments { get; private set; } =
        new List<TrendAttachment>();
    ...
    )
    {
        ...
        Status =
            decision == TrendDecision.ConvertToChangeOrder
    ...
    public void ConvertToChangeOrder(Guid changeOrderId)
    {
        if (Decision != TrendDecision.ConvertToChangeOrder)
            throw new InvalidOperationException(
                "Trend must be approved for conversion to change order"
            );
