...
public class ChangeOrder : BaseEntity, IAuditable, ISoftDelete
{
    ...
    public ICollection<ChangeOrderApproval> Approvals { get; private set; } = new List<ChangeOrderApproval>();
    public ICollection<ChangeOrderImpact> Impacts { get; private set; } = new List<ChangeOrderImpact>();
    ...
    {
        ChangeOrderNumber = changeOrderNumber ?? throw new ArgumentNullException(nameof(changeOrderNumber));
        ...
        string? riskImpact = null)
    {
        if (Status != ChangeOrderStatus.Draft && Status != ChangeOrderStatus.UnderReview)
            throw new InvalidOperationException("Can only assess impact for draft or under review change orders");
    ...
    public void Reject(string rejectedById, string reason)
    {
        if (Status == ChangeOrderStatus.Approved || Status == ChangeOrderStatus.Implemented)
            throw new InvalidOperationException("Cannot reject approved or implemented change orders");
    ...
    public void UpdateContingencyUsed(decimal amount)
    {
        if (Status != ChangeOrderStatus.Approved && Status != ChangeOrderStatus.InProgress)
            throw new InvalidOperationException("Can only update contingency for approved change orders");
    ...
    private void ValidateChangeOrderNumber()
    {
        ...
        if (!System.Text.RegularExpressions.Regex.IsMatch(ChangeOrderNumber, @"^CO-\d{3}-\d{3}$"))
            throw new ArgumentException("Change order number must follow format CO-XXX-### (e.g., CO-001-001)");
    }
    ...
    private ChangeOrderPriority DeterminePriority(decimal costImpact, int scheduleImpactDays)
    {
        ...
        return totalScore switch
        {
            ...
            4 => ChangeOrderPriority.High,
            3 => ChangeOrderPriority.Medium,
            2 => ChangeOrderPriority.Low,
            _ => ChangeOrderPriority.VeryLow
        };
    }
}