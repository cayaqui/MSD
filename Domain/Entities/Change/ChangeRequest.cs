...
public class ChangeRequest : BaseEntity, IAuditable, ISoftDelete, ICodeEntity
{
    ...
    // Calculated Fields
    public int DaysInReview => ReviewStartDate.HasValue
        ? (DateTime.UtcNow - ReviewStartDate.Value).Days
        : 0;

    public bool IsOverdue => Status != ChangeRequestStatus.Closed
        && Status != ChangeRequestStatus.Cancelled
        && DateTime.UtcNow > RequiredByDate;

    public decimal? CostVariance => ActualCostImpact.HasValue && ApprovedCostImpact.HasValue
    ...
    public ICollection<ChangeRequestAttachment> Attachments { get; private set; } = new List<ChangeRequestAttachment>();
    public ICollection<ChangeRequestComment> Comments { get; private set; } = new List<ChangeRequestComment>();
    public ICollection<ChangeRequestApproval> Approvals { get; private set; } = new List<ChangeRequestApproval>();
        ...
        bool safetyImpact)
    {
        ...
        var impactCount = new[] { costImpact, scheduleImpact, qualityImpact, scopeImpact, safetyImpact }
            .Count(x => x);
        ...
        string? costBreakdown = null)
    {
        ...
        RequiredApprovalLevel = Math.Max(RequiredApprovalLevel, DetermineApprovalLevel(estimatedCost));
    ...
    {
        TechnicalJustification = justification ?? throw new ArgumentNullException(nameof(justification));
        ProposedSolution = proposedSolution ?? throw new ArgumentNullException(nameof(proposedSolution));
    ...
    public void RecordTechnicalReview(
        Guid reviewerId,
        bool approved,
        string comments)
    {
        ...
        string? conditions = null)
    {
        if (Status != ChangeRequestStatus.UnderReview && Status != ChangeRequestStatus.Submitted)
            throw new InvalidOperationException("Change request must be under review to be approved");
    ...
    public void Reject(Guid rejectedById, string reason)
    {
        if (Status == ChangeRequestStatus.Approved || Status == ChangeRequestStatus.Implemented)
            throw new InvalidOperationException("Cannot reject an approved or implemented change request");
    ...
    public void RecordClientDecision(
        bool approved,
        string? referenceNumber,
        string? comments)
    {
        if (!RequiresClientApproval)
            throw new InvalidOperationException("This change request does not require client approval");
    ...
    {
        if (Status != ChangeRequestStatus.Approved && Status != ChangeRequestStatus.ReadyToImplement)
            throw new InvalidOperationException("Only approved change requests can be converted to change orders");
        ...
        string? notes = null)
    {
        if (!IsConvertedToChangeOrder)
            throw new InvalidOperationException("Change request must be converted to change order before implementation");
    ...
    public void Cancel(string reason)
    {
        if (Status == ChangeRequestStatus.Implemented || Status == ChangeRequestStatus.Closed)
            throw new InvalidOperationException("Cannot cancel an implemented or closed change request");
    ...
    public void Close(string? lessonsLearned = null)
    {
        if (Status != ChangeRequestStatus.Implemented && Status != ChangeRequestStatus.Rejected)
            throw new InvalidOperationException("Only implemented or rejected change requests can be closed");
    ...
    private void ValidateReadyForSubmission()
    {
        ...
        if (errors.Any())
            throw new InvalidOperationException($"Change request is not ready for submission: {string.Join(", ", errors)}");
    }
