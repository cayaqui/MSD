...
namespace Domain.Entities.Projects
{
    ...
    public class Project : BaseEntity, IAuditable, ISoftDelete, IActivatable
    {
        ...
        // Business Methods
        public void UpdateBasicInfo(string name, string? description, string? location, string? client, string? contractNumber, string currency)
        {
            Name = name ?? throw new ArgumentNullException(nameof(name));
        ...
        public void UpdateBudget(decimal totalBudget, decimal? approvedBudget = null, decimal? contingencyBudget = null)
        {
            if (totalBudget < 0)
        ...
        public void UpdateStatus(ProjectStatus newStatus)
        {
            ...
            else if ((newStatus == ProjectStatus.Completed || newStatus == ProjectStatus.Cancelled) && !ActualEndDate.HasValue)
            {
        ...
        {
            ProjectManagerId = projectManagerId ?? throw new ArgumentNullException(nameof(projectManagerId));
            ProjectManagerName = projectManagerName ?? throw new ArgumentNullException(nameof(projectManagerName));
            UpdatedAt = DateTime.UtcNow;
        }
        ...
        public void SetBaseline(DateTime baselineDate)
        {
            if (Status != ProjectStatus.Planning && Status != ProjectStatus.Active)
                throw new InvalidOperationException("Can only baseline projects in Planning or Active status");
        ...
        {
            if (!ActualStartDate.HasValue) return null;
